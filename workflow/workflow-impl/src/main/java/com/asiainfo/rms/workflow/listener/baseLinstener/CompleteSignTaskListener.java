 ...
 *
 */
public abstract class CompleteSignTaskListener extends WorkflowBaseListener implements TaskListener {

	private final static transient Log log = LogFactory.getLog(CompleteSignTaskListener.class);
	/**
	 * 
	 */
	...
	public void notify(DelegateTask task) {
		...
		try {
			...
			List rtnList = this.signRearTask(Long.valueOf(deployId),processKey,Long.valueOf(task.getId()), objId,objType);
			if(rtnList!=null){
				this.extraMethod(Long.valueOf(rtnList.get(0).toString()), objId, objType, "signRearTask");
			}
		} catch (Exception e) {
